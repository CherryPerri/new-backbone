...
//            }
   ...
   ********************/
        ...
        setCountry: function(countryCode) {
            ...
            var countryData = this._getCountryData(countryCode, false, false)
            this.updateDialCode(countryData["dialCode"]);
        },
        getCountryDialCode : function(dialCode) {
            var countryList = this.countries;
            for (var i = 0; i < countryList.length; i++) {
                if (countryList[i].dialCode == dialCode) {
                    return countryList[i];
                }
            }
            return {};
        ...
        setCountryDialCode : function(dialCode) {
            var countryData = this.getCountryDialCode(dialCode);
            this.setCountry(countryData["iso2"]);
        },
        updateDialCode : function(dialCode) {
            var length = dialCode.length;
            var width = lengthDialCode[length];
            if(width != undefined) {
                $("#txtContactNumber").css("paddingLeft", width);
            }
            $(".selected-dial-code").html("+" + dialCode);
        },
        getContactNumber : function() {
            var countryData = this.getSelectedCountryData();
            var contactNumber = "+" + countryData["dialCode"];
        ...
        setContactNumber : function(number) {
            ...
            $("#txtContactNumber").val(number.replace(dialCode,''));
        },
