...
//                targetSelect.attr("data-placeholder", placeHolder);
            ...
            var allowClear = options["allow-clear"];
            
            if(selectedKey != undefined && selectedKey != "") {
                targetSelect.select2({
                    allowClear : allowClear
                }).select2("val", selectedKey);
            } else {
                targetSelect.select2({
                    allowClear : allowClear
                });
...
var EMSValidate = function() {
    ...
    jQuery.validator.addMethod("validate_host_new", function(value, element) {
      return ValidationUtil.validate_an_except(value, [CHARACTER_DOT]);
    },"Vui l�ng nh?p d�ng d?nh d?ng host. V� d?: 10.0.0.1 ho?c test.abc.com");
    // thanhuy.nguyen ADD START
...
//    	var ipAddressFormat =  "^([1-9]|[1-9][0-9]|1[0-9][0-9]|2[0-4][0-9]|25[0-5])(\.([0-9]|[1-9][0-9]|1[0-9][0-9]|2[0-4][0-9]|25[0-5])){3}$";
   ...
   jQuery.validator.addMethod("ems_validate_decimalPlaces_length", function(value, element) {
    ...
    jQuery.validator.addMethod("validate_connect_Time_lora_sigfox", function(value, element) {
        var result = false;
       value = parseInt(value);
       ...
       return result;
    }, "Please enter valid connect Time ,connect Time input should be used 300 seconds to 86400 msseconds.");
    jQuery.validator.addMethod("validate_wait_connect_Time_lora_sigfox", function(value, element) {
        var result = false;
       value = parseInt(value);
       ...
       return result;
    }, "Please enter valid wait connect Time ,wait connect Time input should be used 300 seconds to 86400 msseconds.");
